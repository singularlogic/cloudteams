...
$(document).ready(function () {
    ...
    $(document).ajaxStart(function () {
        $("#ct-content-github").length > 0 ? $("#ct-content-github").html(getLoadingHTML()) : null;
        $("#ct-content-sonarqube").length > 0 ? $("#ct-content-sonarqube").html(getLoadingHTML()) : null;
        $("#ct-content-bitbucket").length > 0 ? $("#ct-content-bitbucket").html(getLoadingHTML()) : null;
        $("#ct-content-jira").length > 0 ? $("#ct-content-jira").html(getLoadingHTML()) : null;
        $("#ct-content-paasport").length > 0 ? $("#ct-content-paasport").html(getLoadingHTML()) : null;
        $("#widget-loading").show();
    });
    //Load Github widget
...
function getJiraChartsData() {
    ...
    $.ajax({
        ...
        if (parseData.code == "SUCCESS") {
            ...
            // Issue Priority
            var issuePriorityHighchartsOpts = jQuery.extend(true, {}, global_highcharts_options);
            ...
            console.log("Status: " + status + ". Something went wrong!");
            $("#widget-loading").remove();
            customModal("Info", "", "There are no data to show for this JIRA project", "OK");
        }
    }).error(function(data, textStatus, jqXHR) {
        console.log("something went wrong");
    });
